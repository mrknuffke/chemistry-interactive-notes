...
**Status: proposed. Figure tiers decided (§1) and Phase 1 is done (see `TODO.md` / `CHANGELOG.md`). Phase 2 is CLOSED (2026-07-07) — the full §2 deployment map is deployed: 1-1a (step, shell-fill extension), 1-1b (skipped, existing interactives already cover it), 1-2a (scrub), 1-2b (step), 1-3a (step), 1-3b (scrub), 2-2a (step ×2, Before/During/After), 2-7a (zoom, pre-existing, left alone), 2-7b (step), 2-7c (step), C-RXN (step, extended to the reaction-type classifier), C-SPA (zoom, fat melt/re-solidify lamination — mechanism verified against Unit 2 Consensus Notes Arc 3). Structural HTML-parser + marker/id-uniqueness sweep run clean across all 12 lessons. Several real bugs found and fixed along the way (see `CHANGELOG.md`): the `--dia-label-size`/`--dia-caption-size` unitless-token bug (site-wide, `tokens.css`), bond-over-atom rendering in 1-2b's H2O/CH4 explorer, an undefined `static-molar-table` class in 2-7c. Author-approved closed without a further per-lesson browser pass. Remaining author calls: §7 items 4 and 6 (block Phases 4 and 3 respectively).**
