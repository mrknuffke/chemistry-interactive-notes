...
## 1a. Diagram standardization — CLOSED (2026-07-08)

**Spec:** [`DIAGRAM_STANDARDIZATION.md`](DIAGRAM_STANDARDIZATION.md). **Contract:** [`DIAGRAM_CONTRACT.md`](DIAGRAM_CONTRACT.md).

A design-level retrofit that fixes the "janky diagrams" problem at a deeper layer than `REVISION_PLAN.md` Phase 1: within-figure breakage first (make every diagram one self-contained `<svg viewBox>` that scales as a rigid unit), then cross-figure drift (a shared `--dia-*` token layer so an oxygen atom is the same size everywhere), then data-driven renderers. `REVISION_PLAN.md` Phase 1 sized figures (outer width, done); this plan fixes their internals. All phases (0–4) are now closed.

**Done (2026-07-08):**
- **Phase 3 (coordinate-scale retrofit), batch 3 — `1-1a`, `2-7c`, `C-RXN`, `C-SPA`.** Retrofitted final 4 lessons to the `--dia-*` design tokens, conformed colors (Oxygen to `var(--accent)`, Nitrogen/Chlorine to `var(--ink-mute)`, Hydrogen to `#ffffff`), and resolved Chrome font-size attribute parsing bugs via inline styles.
- **Phase 4 (data-driven renderers)** — Built the shared Molecule Renderer (`scaffold/assets/molecule-renderer.js`) and Particle-Scene Renderer (`scaffold/assets/particle-renderer.js`), wired up validations (particle count conservation check), and added rendering demos in `_widget-test.html`.
- **Learner Dashboard Concept Map** — Implemented the visual, color-coded SVG Concept Map tab on the index dashboard (`scaffold/index.html`) reacting to dynamic checkpoint progress.
