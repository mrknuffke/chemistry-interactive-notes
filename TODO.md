...
A design-level pass on top of the (already-closed) bug fixes — a shared figure-sizing system, deploying the step/zoom/scrub motion primitives that exist but are barely used, breaking the "produce → reveal" interaction monotony, and making lessons re-enterable (persistence + synopsis) instead of read-once. Phase 1 (the figure system) shipped 2026-07-06. Phase 2 (motion deployment) sign-off items (§7 items 2, 3, 5) resolved 2026-07-07; deployment progress so far: `1-1a`'s step primitive extended from 4 to 6 steps (K→Ga shell-filling); `1-2a` got a new `scrub` widget (ΔEN 0–3.5 drags a shared electron cloud from centered → lopsided → gone, with polar/ionic threshold marks); `1-2b`'s hero diagram became a 3-step sequence (bonds are polar → each gets a vector → vectors sum to the molecular result), with a new `bondBetween()` helper so bond lines clip to the atom's edge instead of running under it (implements `DIAGRAM_CONTRACT.md` §9's clipping rule); `1-1b` was deliberately **skipped** — the reference-build lesson already has two rich bespoke interactives covering period/group trends, and the one real scrub candidate (the `s-reactivity` compass) was deferred rather than retrofitted this pass. `2-2a`'s two particle-diagram panels (ice melting, hydrogen combustion — the map's `2-2a` row) also became 3-step Before/During/After sequences; the combustion "During" frame is a genuine pedagogical addition (shows unreacted pairs, an atom-conserved mid-reaction cluster with dashed forming bonds, and already-formed product side by side) not just a mechanical split of the old two-panel static diagram. **Incomplete:** `core.js` has a `window.GC_SCRUB_FN['imf-cooling-curve']` registered for `1-3b`'s evaporation-rate graph (the map's `1-3b` row), but the corresponding markup change (a `data-motion="scrub"` wrapper + `.imf-curve`/`.imf-curve-label` classes on the graph) was never applied — the function is currently dead code. Finish or remove it before the next `1-3b` touch. Remaining map rows in `REVISION_PLAN.md` §2 are still outstanding. Phases 3–6 still have open author sign-off items (§7 items 4, 6) — check those before executing the phase they block.
